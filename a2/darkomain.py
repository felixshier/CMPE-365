...
def getNewStartingTriangle(OneSidedTrianglesLeft, TwoSidedTrianglesLeft, ThreeSidedTrianglesLeft):
    if len(OneSidedTrianglesLeft)>0:
        return OneSidedTrianglesLeft[0]
    elif len(TwoSidedTrianglesLeft)>0:
        return TwoSidedTrianglesLeft[0]
    elif len(ThreeSidedTrianglesLeft)>0:
        return ThreeSidedTrianglesLeft[0]
        
def buildTristrips( triangles ):

    count = 0
    #trianglesLeft = triangles.copy()
    #trianglesLeft.sort(key = lambda x: len(x.adjTris))
    ThreeSidedTrianglesLeft = []
    TwoSidedTrianglesLeft = []
    OneSidedTrianglesLeft = []
    for index in range(len(triangles)):          ##Add triangles to a list depending on number of adjTriangles they have
        if len(triangles[index].adjTris)==3:
            ThreeSidedTrianglesLeft.append(triangles[index])
        elif len(triangles[index].adjTris)==2:
            TwoSidedTrianglesLeft.append(triangles[index])
        elif len(triangles[index].adjTris)==1:
            OneSidedTrianglesLeft.append(triangles[index])
        else: 
            print("error")
    currentTri = getNewStartingTriangle(OneSidedTrianglesLeft, TwoSidedTrianglesLeft, ThreeSidedTrianglesLeft)
    # [YOUR CODE HERE]
    #
    # Increment 'count' every time you *start* a new triStrip.
    
    while(len(ThreeSidedTrianglesLeft) + len(TwoSidedTrianglesLeft) + len(OneSidedTrianglesLeft) > 0):
        for i in range(len(currentTri.adjTris)):
            #This is new part i just added
            if currentTri.adjTris in ThreeSidedTrianglesLeft:
                ThreeSidedTrianglesLeft.remove(currentTri.adjTris)
                TwoSidedTrianglesLeft.add(currentTri.adjTris)
            elif currentTri.adjTris in TwoSidedTrianglesLeft:
                Two.remove(currentTri.adjTris)
                OneSidedTrianglesLeft.add(currentTri.adjTris)
            else:
                OneSidedTrianglesLeft.remove(currentTri.adjTris)
            ##ends here
